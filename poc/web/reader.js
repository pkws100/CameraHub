...
class MediaMTXWebRTCReader {
  ...
  #sendOffer(offer) {
    ...
    return fetch(this.#conf.url, {
      ...
      if (location === null) {
        throw new Error("WHEP session location missing");
      }
      const sessionUrl = new URL(location, this.#conf.url);
      const endpointUrl = new URL(this.#conf.url);
      if (
        endpointUrl.pathname.startsWith("/whep/") &&
        sessionUrl.origin === endpointUrl.origin &&
        !sessionUrl.pathname.startsWith("/whep/")
      ) {
        sessionUrl.pathname = `/whep${sessionUrl.pathname}`;
      }
      this.#sessionUrl = sessionUrl.toString();
